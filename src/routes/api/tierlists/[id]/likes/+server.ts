...
async function count(db: D1Database, tierlistId: string) {
	const row = await db
		.prepare(
		...
		.first<{ count: number }>();
	await db
		.prepare('UPDATE tierlists SET likes = ? WHERE id = ?')
		.bind(row?.count ?? 0, tierlistId)
		.run();
	return row?.count ?? 0;
}
...
export const GET: RequestHandler = async ({ params, url, platform }) => {
	...
	const row = userId
		? await db
				.prepare(
					"SELECT 1 FROM likes WHERE content_type = 'tierlist' AND content_id = ? AND user_id = ?"
				)
...
export const POST: RequestHandler = async ({ params, request, platform }) => {
	...
	await db
		.prepare(
			"INSERT OR IGNORE INTO likes (content_type, content_id, user_id) VALUES ('tierlist', ?, ?)"
		)
		.bind(params.id, userId)
		.run();
...
export const DELETE: RequestHandler = async ({ params, request, platform }) => {
	...
	const body = await request.json().catch(() => ({}));
	await db
		.prepare("DELETE FROM likes WHERE content_type = 'tierlist' AND content_id = ? AND user_id = ?")
